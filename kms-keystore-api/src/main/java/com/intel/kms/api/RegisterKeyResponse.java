 ...
 * superclass is populated with the complete set of attributes for the
 * created key; this will reflect attributes specified by the client
 * (and permitted by policy) as well as any attributes added automatically
 * by the server.  The attributes are the same as what would be received
 * if the client were to issue a follow-up request to /keys/{keyId}, and
 * they are provided to allow the client to avoid making the extra request.
 * 
 * @author jbuhacoff
 */
