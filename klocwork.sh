...
export KLOCWORK_PROJECT=dcg_security-kms
export KLOCWORK_SERVER_URL=https://klocwork-jf18.devtools.intel.com:8160
export MAIN_PROJECT_SPEC=kms.out
...
execPrereqCheck(){
   ...
   if [ -z "$KLOCWORK_PROJECT" ]; then
      echo_failure "KLOCWORK_PROJECT Env. variable is missing (i.e. dcg_security-kms)"
#      return 1
	check=1
   fi
...
#      return 1
   ...
   if [ -z "$MAIN_PROJECT_SPEC" ]; then
      echo_failure "MAIN_PROJECT_SPEC Env. variable is missing (i.e. kms.out)"
#      return 1
	check=1
   fi
}
...
displayUsage() {
	...
	echo "Usage:"
	echo -e "This script allows you to run the klocwork scan for the kms projects. The script needs the following environment variables set before running: \n"
	echo -e " *BUILD_SPECS_DIRECTORY (i.e. /home/klocwork/kw_build_specs) \n *TABLES_DIRECTORY (i.e. /home/klocwork/kwtables) \n *KLOCWORK_PROJECT (i.e. dcg_security-kms) \n *KLOCWORK_SERVER_URL (i.e. https://klocwork-jf18.devtools.intel.com:8160) \n *MAIN_PROJECT_SPEC (i.e. kms.out) \n"
