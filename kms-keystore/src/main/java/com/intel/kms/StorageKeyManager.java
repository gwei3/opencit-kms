...
//        this.mapper = JacksonObjectMapperProvider.createDefaultMapper();
    ...
    private KeyAttributes getDefaultKeyAttributes() {
        ...
        keyAttributes.digestAlgorithm = "SHA-256";
//        keyAttributes.id;
        keyAttributes.setKeyLength(128);
//        keyAttributes.name;
//        keyAttributes.paddingMode;
        keyAttributes.role = "keyEncryption";
//        keyAttributes.transferPolicy;  // no transfer policy because this key is not transferable;  maybe this should be a urn with "private" at the end.
        log.debug("getDefaultKeyAttributes algorithm {} length {} cipher mode {} digest algorithm {}", keyAttributes.getAlgorithm(), keyAttributes.getKeyLength(), keyAttributes.getMode(), keyAttributes.digestAlgorithm);
    ...
    public KeyAttributes createStorageKey() throws KeyStoreException {
        try {
            ...
            keyAttributes.id = new UUID().toString();

            // just in case the keystore already has an entry with this id:
            while (keystore.contains(keyAttributes.id)) {
                log.warn("Duplicate UUID detected: {}", keyAttributes.id);
                keyAttributes.id = new UUID().toString();
            }
...
//            log.debug("Creating storage key: {}", mapper.writeValueAsString(keyAttributes)); // throws IOException
            log.debug("Creating storage key id {} algorithm {} length {} cipher mode {} digest algorithm {}", keyAttributes.id, keyAttributes.getAlgorithm(), keyAttributes.getKeyLength(), keyAttributes.getMode(), keyAttributes.digestAlgorithm);
            ...
            keystore.set(keyAttributes.id, skey);
