...
public class TpmIdentityCertificateRepository implements DocumentRepository<Certificate, CertificateCollection, CertificateFilterCriteria, CertificateLocator> {
    ...
    public static final String MTWILSON_TPM_IDENTITY_CERTIFICATES_PASSWORD = "tpm_identity_certificates"; // the alias of the password in the password vault, not the password itself
    public static final String MTWILSON_TPM_IDENTITY_CERTIFICATES_FILE_TYPE = "JKS";
//    private Configuration configuration;
    ...
    private void configure(Configuration configuration) {
        ...
        this.keystoreFile = new File(configuration.get(MTWILSON_TPM_IDENTITY_CERTIFICATES_FILE, Folders.configuration() + File.separator + "tpm.identity.jks"));
        ...
        try (PasswordKeyStore passwordVault = PasswordVaultFactory.getPasswordKeyStore(configuration)) {
            if (passwordVault.contains(MTWILSON_TPM_IDENTITY_CERTIFICATES_PASSWORD)) {
                this.keystorePassword = passwordVault.get(MTWILSON_TPM_IDENTITY_CERTIFICATES_PASSWORD); // throws exception if password is not in vault
            } else {
                //faults.add(new PasswordVaultEntryNotFound(MTWILSON_TPM_IDENTITY_CERTIFICATES_PASSWORD));
                log.error("Password vault entry not found: {}", MTWILSON_TPM_IDENTITY_CERTIFICATES_PASSWORD);
...
//            faults.add(new PasswordVaultUnavailable(e));
    ...
    public List<X509Certificate> getCertificates() throws KeyStoreException, IOException {
        try (PublicKeyX509CertificateStore keystore = new PublicKeyX509CertificateStore(MTWILSON_TPM_IDENTITY_CERTIFICATES_FILE_TYPE, new ExistingFileResource(keystoreFile), keystorePassword)) {
...
//        try (CertificateDAO dao = TagJdbi.certificateDao()) {
        try (PublicKeyX509CertificateStore keystore = new PublicKeyX509CertificateStore(MTWILSON_TPM_IDENTITY_CERTIFICATES_FILE_TYPE, new FileResource(keystoreFile), keystorePassword)) {
            if (keystore.contains(alias)) {
