...
import com.intel.kms.saml.api.fault.ConfigurationUnavailable;
import com.intel.kms.saml.api.fault.PasswordVaultEntryNotFound;
...
public class SamlCertificateRepository implements DocumentRepository<Certificate, CertificateCollection, CertificateFilterCriteria, CertificateLocator> {
    ...
    public static final String MTWILSON_SAML_CERTIFICATES_PASSWORD = "saml_certificates"; // the alias of the password in the password vault, not the password itself
    public static final String MTWILSON_SAML_CERTIFICATES_FILE_TYPE = "JKS";
//    private Configuration configuration;
    private File keystoreFile;
    private Password keystorePassword = null;

    public SamlCertificateRepository() throws IOException {
        configure(ConfigurationFactory.getConfiguration());
    }

    public SamlCertificateRepository(Configuration configuration) {
        configure(configuration);
    }

    private void configure(Configuration configuration) {
        // locate the keystore file that has the trusted saml certificate authorities
        this.keystoreFile = new File(configuration.get(MTWILSON_SAML_CERTIFICATES_FILE, Folders.configuration() + File.separator + "saml.jks"));
        // get the password for the saml keystore from the password vault
        try (PasswordKeyStore passwordVault = PasswordVaultFactory.getPasswordKeyStore(configuration)) {
            if (passwordVault.contains(MTWILSON_SAML_CERTIFICATES_PASSWORD)) {
                this.keystorePassword = passwordVault.get(MTWILSON_SAML_CERTIFICATES_PASSWORD); // throws exception if password is not in vault
            } else {
                //faults.add(new PasswordVaultEntryNotFound(MTWILSON_SAML_CERTIFICATES_PASSWORD));
                log.error("Password vault entry not found: {}", MTWILSON_SAML_CERTIFICATES_PASSWORD);
...
//            faults.add(new PasswordVaultUnavailable(e));
    ...
    public List<X509Certificate> getCertificates() throws KeyStoreException, IOException {
        try (PublicKeyX509CertificateStore keystore = new PublicKeyX509CertificateStore(MTWILSON_SAML_CERTIFICATES_FILE_TYPE, new ExistingFileResource(keystoreFile), keystorePassword)) {
    ...
    @Override
    @RequiresPermissions("saml_certificates:search")
    public CertificateCollection search(CertificateFilterCriteria criteria) {
        log.debug("SamlCertificateRepository search");
...
//            Result<Record> result = sql.fetch();
    ...
    @Override
    @RequiresPermissions("saml_certificates:retrieve")
    public Certificate retrieve(CertificateLocator locator) {
        log.debug("SamlCertificateRepository retrieve");
    ...
    @Override
    @RequiresPermissions("saml_certificates:store")
    public void store(Certificate item) {
        log.debug("SamlCertificateRepository store");
        throw new UnsupportedOperationException();

    }

    @Override
    @RequiresPermissions("saml_certificates:create")
    public void create(Certificate item) {
        log.debug("SamlCertificateRepository create");
        CertificateLocator locator = new CertificateLocator();
        locator.id = item.getId();
        String alias = locator.id.toString();
//        try (CertificateDAO dao = TagJdbi.certificateDao()) {
        try (PublicKeyX509CertificateStore keystore = new PublicKeyX509CertificateStore(MTWILSON_SAML_CERTIFICATES_FILE_TYPE, new FileResource(keystoreFile), keystorePassword)) {
            if (keystore.contains(alias)) {
    ...
    @Override
    @RequiresPermissions("saml_certificates:delete")
    public void delete(CertificateLocator locator) {
        log.debug("SamlCertificateRepository delete with locator");
        if (locator == null || locator.id == null) {
            return;
        }
        String alias = locator.id.toString();
    ...
    @Override
    @RequiresPermissions("saml_certificates:delete,search")
    public void delete(CertificateFilterCriteria criteria) {
        log.debug("SamlCertificateRepository delete with criteria");
